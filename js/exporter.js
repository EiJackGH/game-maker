...
/**
 * Generates a standalone Loader HTML bundle that includes game runtime and a level JSON file loader.
 */
function generateLoaderHTMLBundle() {
  const currentGridJSON = JSON.stringify(state.grid);
  const customBlocksJSON = JSON.stringify(state.customBlocks);
  const exportedDefaultBlocks = JSON.stringify(DEFAULT_BLOCKS);
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Blocks Retro Game Loader</title>
  <!-- Tailwind CSS -->
  <script src="https://cdn.tailwindcss.com"></script>
  <!-- FontAwesome Icons -->
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
...
<body class="bg-gray-950 text-gray-100 min-h-screen flex flex-col items-center justify-center p-4 font-sans selection:bg-purple-600 selection:text-white">
  ...
  <div class="w-full max-w-4xl bg-gray-900 border border-purple-500/30 rounded-xl shadow-2xl p-4 md:p-6 space-y-4">
    <!-- Header info -->
    <div class="flex flex-col sm:flex-row items-center justify-between border-b border-gray-800 pb-3 gap-3 sm:gap-0">
      <div class="flex items-center space-x-3">
        <div class="bg-purple-600 text-white p-2 rounded-lg flex items-center justify-center shadow-lg shadow-purple-900/30">
          <i class="fas fa-gamepad text-lg"></i>
        </div>
        <div>
          <h1 class="font-bold text-lg tracking-wide bg-gradient-to-r from-purple-400 to-indigo-400 bg-clip-text text-transparent">BLOCKS: GAME LOADER</h1>
          <p class="text-[10px] text-gray-400 font-mono">Standalone Level Loader & Player</p>
        </div>
      </div>
    ...
    <!-- Level File Loader Control Card & Dropzone -->
    <div id="drop-zone" class="bg-gray-950/80 border-2 border-dashed border-gray-700 hover:border-purple-500/80 rounded-xl p-4 text-center transition cursor-pointer space-y-2">
      <input type="file" id="loader-file-input" accept=".json" class="hidden" />
      <div class="flex items-center justify-center space-x-3">
        <button id="btn-browse-file" class="px-3 py-1.5 bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs rounded-lg transition shadow flex items-center space-x-1.5">
          <i class="fas fa-folder-open"></i>
          <span>Choose Level JSON File</span>
        </button>
        <span class="text-xs text-gray-400 font-mono hidden sm:inline">or drag & drop level .json file here</span>
      </div>
      <p id="loader-status" class="text-xs text-purple-300 font-mono">Currently playing default/loaded level. Select a new JSON file to load another level.</p>
    </div>
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between text-xs text-gray-400 bg-gray-950/40 p-3 rounded-lg border border-gray-800">
      ...
      <div class="mt-2 sm:mt-0 text-[10px] text-purple-400 font-mono">
        Built with <a href="https://github.com" target="_blank" class="underline">Blocks Game Maker</a>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    function loadLevelData(jsonData, fileName) {
      try {
        if (!jsonData || !jsonData.grid || !Array.isArray(jsonData.grid)) {
          alert("Invalid level file format. Please load a valid Blocks level JSON file.");
          return;
        }
        state.cols = jsonData.cols || jsonData.grid[0].length;
        state.rows = jsonData.rows || jsonData.grid.length;
        state.grid = jsonData.grid;
        state.customBlocks = jsonData.customBlocks || {};
        state.gravity = jsonData.gravity !== undefined ? jsonData.gravity : 0.5;
        state.speed = jsonData.speed !== undefined ? jsonData.speed : 4;
        state.lives = jsonData.lives !== undefined ? jsonData.lives : 3;
        state.genre = jsonData.genre || "platformer";

        const statusElem = document.getElementById("loader-status");
        if (statusElem) {
          statusElem.textContent = "Loaded level: " + (fileName || "level.json") + " (" + state.cols + "x" + state.rows + " " + state.genre + ")";
          statusElem.className = "text-xs text-emerald-400 font-mono font-bold";
        }

        game.start();
      } catch (err) {
        console.error("Error parsing level JSON:", err);
        alert("Failed to parse level JSON file: " + err.message);
      }
    }

    function setupFileLoader() {
      const fileInput = document.getElementById("loader-file-input");
      ...
      function handleFile(file) {
        if (!file.name.endsWith(".json")) {
          alert("Please select a valid .json level file!");
          return;
        }
        ...
        reader.onload = (event) => {
          try {
            ...
            loadLevelData(parsed, file.name);
          } catch (err) {
            alert("Error reading level JSON file: " + err.message);
          }
        };
  ...
  const dlAnchorElem = document.createElement('a');
  dlAnchorElem.setAttribute("href", blobUrl);
  dlAnchorElem.setAttribute("download", "loader.html");
  dlAnchorElem.click();
...
const btnDownloadLoader = document.getElementById("btn-download-loader");
if (btnDownloadLoader) {
  btnDownloadLoader.addEventListener("click", generateLoaderHTMLBundle);
}
